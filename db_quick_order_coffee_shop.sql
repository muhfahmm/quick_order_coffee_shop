...
CREATE TABLE `tb_products` (
  ...
  `hot_name` VARCHAR(150) NULL,
  `hot_image` VARCHAR(255) NULL,
  `hot_price` DECIMAL(12, 2) NULL,
  `hot_available` TINYINT(1) DEFAULT 1,
  `ice_name` VARCHAR(150) NULL,
  `ice_image` VARCHAR(255) NULL,
  `ice_price` DECIMAL(12, 2) NULL,
  `ice_available` TINYINT(1) DEFAULT 1,
...
CREATE TABLE `tb_orders` (
  ...
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  CONSTRAINT `fk_orders_table` FOREIGN KEY (`table_id`) REFERENCES `tb_tables` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
